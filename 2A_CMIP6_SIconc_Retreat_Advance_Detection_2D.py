...
Date Created: 6 Sep 2019
Date Modified: 18 May 2020
25 Aug 2020 --> Modified the time treatment so that it is more flexible and can handle
    models that keep going past the desired end time. Also fixed a bug in the units for time.
...
Inputs:
    ...
    the moving average size (n) -- b/c of bi-daily data in the 1980s, 5 or 7
    is recommended.
    months in which to search for  maximum (maxmo) and minimum (minmo) values
    last year wih valid data (ymax)
    experiment for data (experiment) -- e.g., "ssp585", "historical"
...
### Time Variables ###
ymax = 2099  # Last year of FULL data, meaning you need to have Jan, Feb, Mar of subsequent year, too
maxmo = [1,4] # months in which the sea ice maximum may occur
minmo = [8,10] # months in which the sea ice minimum may occur

### Path Variables ###
experiment = 'ssp370'
path = "/project/6061839/crawfora/CMIP6/SeaIce/" # "/Users/acrawfora/Documents/CMIP6/SeaIce" #  # "/Volumes/Cassandra/CMIP6/SeaIce"  #
inpath = path+"/SmoothedMA"+str(n)+"/"+experiment
outpath = path+"/AdvanceRetreat/"+experiment+"/C"+str(concThresh)
...
print("Main Analysis")
# Load file names
files = md.listdir(inpath)
files = [f for f in files if ((f.startswith("AWI") == 0) & (f.startswith("ICON") == 0))]

cmplfiles = md.listdir(outpath)

newfiles = [f for f in files if f not in cmplfiles]
newfiles.sort()
print("New: " + str(newfiles))

with open(path+'/Code2_'+experiment+'_newfiles.txt', 'w') as txt:
    txt.write(str(newfiles))

# Run main code
for f in newfiles[:]:
    ncfiles = md.listdir(inpath+"/"+f)
    ...
    for fi in range(len(ncfiles)):
        ...
        with open(path+'/Code2_'+ncfiles[fi]+'.txt', 'w') as txt:
            txt.write(str("Starting " + ncfiles[fi]))
        try:
            ncf = nc.Dataset(inpath+"/"+f+"/"+ncfiles[fi])
        except:
            ncf = xr.open_dataset(inpath+"/"+f+"/"+ncfiles[fi],decode_times=False)

        # Determine Start Year & Leap Years
        times = ncf.variables['time'][:]
        reftime = [int(ncf['time'].units[11:15]),1,1,0,0,0]
        ...
        for y in years:
            ...
            if y == np.max(years) and fi+1 < len(ncfiles):
                try:
                    ...
                    ncf2 = xr.open_dataset(inpath+"/"+f+"/"+ncfiles[fi+1], decode_times=False)
                    
                reftime2 = [int(ncf2['time'].units[11:15]),1,1,0,0,0]
    ...
    ### Write Outputs ###
    outName = "siphenology_C"+str(concThresh)+"_"+f+"_"+str(int(yearsnc[0]))+"-"+str(int(yearsnc[-1]))+".nc"
    try:
        os.chdir(outpath+"/"+f)
    except:
        os.mkdir(outpath+"/"+f)
