...
### Time Variables ###
ymin, ymax = 1979, 2017 # Years of interest
...
files = [f for f in files if f.startswith('nt')]
files.sort()
...
for y in years:
    ...
    try: # If a smoothed SIC array already exists, open it
        ...
        for f in fileyList_0 + fileyList + fileyList_2:
            ...
            try:
                arr0 = md.reclassIceArray( np.fromfile(inpath+'/'+f,dtype='uint8')[300:].reshape((448,304)) )
            except:
                arr0 = md.reclassIceArray( np.fromfile(inpath2+'/'+f,dtype='uint8')[:].reshape((448,304)) )
    ...
    for i in range(arr.shape[0]):
        holeR, holeC = np.where((lats >= 84.6) & (np.isnan(arr[i,:,:])))
