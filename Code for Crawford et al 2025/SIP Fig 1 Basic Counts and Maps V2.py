...
# inpath2 = '/home/Datapool/SeaIce'
inpath = '/Users/acrawfora/Library/CloudStorage/OneDrive-UniversityofManitoba/CMIP6 OWP/Sea Ice Extent Expansion Pauses/'
inpath2 = '/Volumes/Miranda/SeaIce'

sicpath = inpath2+'/G02202_V4/Daily'
sicpath2 = inpath2+'/G10016_V2/Daily'
siepath = inpath+'/SIE_Daily_ArcticWide.csv'

outpath = inpath+"/Figures/Fig1_BasicCounts_and_Maps_bytime_V2.png"

dmin, dmax = 276, 428 # 369, 400, 428
dmax2 = 369
ymin, ymax = 1979, 2022
...
siedfyear = siedfall.loc[siedfall['DOY'] < dmax2,['SYear','LowVal']].groupby(by=['SYear']).count().reset_index()
...
xvallabs = xvals
yvals = [274,305,335,366,397,428]
ylab = ['Oct 1','Nov 1','Dec 1','Jan 1','Feb 1','Mar 1']
...
ax.hlines(y=366,xmin=siedfall.iloc[0]['SYear']-1,xmax=siedfall.iloc[-1]['SYear'],color='k', linewidth=0.6, linestyle='solid',zorder=1)
...
titl = ax.annotate('b. All SIE Expansion Pauses (Oct-Feb)', xy=(0.02,0.98), xycoords='axes fraction', va='top', weight='bold', size=8)
...
ax.annotate('# Days per\nMonth', xy=(0.95,-0.015), xycoords='axes fraction', va='top', ha='center', size=7)
...
ax2.set_xticklabels(xvallabs, size=7, rotation=0, ha='center', va='top')
ax2.set_yticks([0,5])
...
titl = ax2.annotate('a. Autumn Pause Days per Year', xy=(0.02,0.98), xycoords='axes fraction', va='top', weight='bold', size=8)
titl.set_path_effects([pe.withStroke(linewidth=3, foreground='w')])
...
for y in range(len(sipyears)):
    ...
    for jd in sipsub['JulianDay']:
        siesub = sie.loc[sie['JulianDay'] == jd-6]
        ax5.scatter(siesub['DOY'],siesub['sie_cdr'], marker='.', s=20, alpha=1, color=colors[y])
...
ax5.annotate('c. Daily SIE Oct-Feb', xy=(0.01,0.98), xycoords='axes fraction', ha='left', va='top', size=8, weight='bold')
ax5.set_xticks(doys)
ax5.set_xticklabels(doyl)
ax5.set_xlim(xmin=274,xmax=428)
...
ax4.annotate('e. On Jan 1 (At End\n                     of Autumn)', xy=(0.02,0.98), xycoords='axes fraction', 
              weight='bold', size=7, ha='left', va='top', color='w', zorder=12)
...
# Legend
ax5.legend(loc='lower right',bbox_to_anchor=(0.83,0.01),fontsize=7, edgecolor='w', framealpha=0,ncols=3)
