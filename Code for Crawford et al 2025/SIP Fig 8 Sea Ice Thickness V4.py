...
sitpaths = [path+'/CS2SMOS-Monthly',path+'/RDEFT4-CryoSat2-30day',path+'/Cryosat2-UBristol/seaicethickness',path+'/ISSITGR4-IceSat-Seasonal',path+'/IS2SITMOGR4-IceSat2-Monthly']
...
sitprjs = ['EASE2-Bremen','PSN','EASE2-Bristol','PSN','PSN']
sats = ['CS2SMOS','CryoSat-2','CryoSat-2','ICESat','ICESat-2']
satvers = [['CS2SMOS'],['RDEFT4'],['UBristol','UBristol-incSMOS'],[''],['NESOSIM v1.1 snow loading','NESOSIM v1.1 snow loading - interpolated','SnowModel-LG snow loading','modified Warren 99 snow loading','bulk ice density parameterization + NESOSIM v1.1 snow loading']]
satvs = [['CS2SMOS'],['RDEFT4'],['UBristol','UBristol-incSMOS'],[''],['NESOSIM','NESOSIM-int','SMLG','mw99','NESOSIM-j22']]
sitvars = [['analysis_sea_ice_thickness'],['sea_ice_thickness'],['Sea_Ice_Thickness','Sea_Ice_Thickness_incSMOS'],['ice_thickness_int'],['ice_thickness','ice_thickness_int','ice_thickness_sm','ice_thickness_mw99','ice_thickness_j22']]
sitlatvars = ['lat','lat','Latitude','latitude','latitude']
sitlonvars = ['lon','lon','Longitude','latitude','latitude']
sitpreyear = ['CS2SMOS_','RDEFT4_','ubristol_cryosat2_seaicethickness_nh25km_','ISSITGR4_01_ON','IS2SITMOGR4_01_']
sityearlen = [4,4,4,2,4] # Number of characters in the year in the filename
sitmons = [['10.nc','11.nc','12.nc'],['1031.nc','1130.nc','1231.nc'],['_10_','_11_','_12_'],['_ON','_ON'],['10_006','11_006','12_006']]
mons = [[10,11,12],[10,11,12],[10,11,12],[10,11],[10,11,12]]

bbox = [80,85,-30,100] # minlat, maxlat, minlon, maxlon

ease2apath = '/Volumes/Miranda/Projections/Cryosat2-UBristol_Projection.nc'
ease2bpath = '/Volumes/Miranda/SeaIce/CS2SMOS-Monthly/CS2SMOS_201110.nc'
...
# Create Mask for EASE2 Grids #
psn = xr.open_dataset(psnpath)
ease2a = xr.open_dataset(ease2apath)
ease2b = xr.open_dataset(ease2bpath)

psn_to_ease2a = xe.Regridder(psn,ease2a,method='bilinear')
psn_to_ease2b = xe.Regridder(psn,ease2b,method='bilinear')

maskease2a = np.where(psn_to_ease2a(probsie_start) >= 0.5, 1, np.nan)
maskease2b = np.where(psn_to_ease2b(probsie_start) >= 0.5, 1, np.nan)

# Write out Masks
psn = psn.assign(mask=(['y','x'],maskpsn))
ease2a = ease2a.assign(mask=(['y','x'],maskease2a))
ease2b = ease2b.assign(mask=(['y','x'],maskease2b))

psnout = psn[['lat','lon','mask','area']]
ease2aout = ease2a[['lat','lon','mask','area']]
ease2bout = ease2b[['lat','lon','mask']]
...
ease2aout.to_netcdf('/Users/acrawfora/Library/CloudStorage/OneDrive-UniversityofManitoba/CMIP6 OWP/Sea Ice Extent Expansion Pauses/masks/ease2-bristol_25km_projection.nc')
ease2bout.to_netcdf('/Users/acrawfora/Library/CloudStorage/OneDrive-UniversityofManitoba/CMIP6 OWP/Sea Ice Extent Expansion Pauses/masks/ease2-bremen_25km_projection.nc')

# Define a mask for each Thickness Dataset Depending on Projection #
sitmasks = [maskpsn if p == 'PSN' else maskease2a if p == 'EASE2-Bristol' else maskease2b for p in sitprjs]
areas = [psn['area'][:].data if p == 'PSN' else ease2a['area'][:].data if p == 'EASE2-Bristol' else np.zeros_like(ease2b['lat'][:].data)+625  for p in sitprjs]
    ...
    for m in range(len(sitmons[s])):
        ...
        for file in mfiles:
            ...
            for v in range(len(satvers[s])):
                ...
                # Append to lists
                mlist.append( mons[s][m] ), ylist.append(int(file.split(sitpreyear[s])[1][0:sityearlen[s]]) )
...
ax1.vlines(x=2004.5, ymin=0, ymax=4.65, color='0.8', linestyle='dashed', linewidth=2, zorder=1)
ax1.vlines(x=2004.5, ymin=0, ymax=4.65, color='k', alpha=0.5, linestyle='dashed', linewidth=1.2, zorder=1)
# ax1.vlines(x=2003.5, ymin=0, ymax=4.5, color='red', alpha=0.5, linestyle='dashed', linewidth=1.2, zorder=1)

ax1.plot(soriot1['Year'],soriot1['AvgSIT-SIEMask'], color='k', label='PMW')
# ax1.plot(soriot2['Year'],soriot2['AvgSIT-SIEMask'], color='red', label='PMW (Oct-Nov)')

df = sitdf2.loc[(sitdf2['Satellite'] == 'CryoSat-2') & (sitdf2['Version'] == 'RDEFT4')]
ax1.plot(df['Year'],df['AvgSIT-SIEMask'], color='orange', label='CS2-NSIDC')

# df = sitdf2.loc[(sitdf2['Satellite'] == 'CryoSat-2') & (sitdf2['Version'] == 'UBristol')]
# ax1.plot(df['Year'],df['AvgSIT-SIEMask'], color='gold', label='CryoSat-2 UBristol')

df = sitdf2.loc[(sitdf2['Satellite'] == 'CS2SMOS') & (sitdf2['Version'] == 'CS2SMOS')]
ax1.plot(df['Year'],df['AvgSIT-SIEMask'], color='darkgreen', label='CS2SMOS')

df = sitdf2.loc[(sitdf2['Satellite'] == 'ICESat-2') & (sitdf2['Version'] == 'NESOSIM-int')]
ax1.plot(df['Year'],df['AvgSIT-SIEMask'], color='magenta', label='ICESat-2')

df = sitdf2.loc[(sitdf2['Satellite'] == 'ICESat')]
ax1.plot(df['Year'],df['AvgSIT-SIEMask'], color='blue', label='ICESat')

ax1.set_xlim(xmin=1989.5,xmax=2024)
ax1.set_ylim(ymin=0.3,ymax=4.65)
ax1.tick_params(axis='both', which='major', labelsize=7)
ax1.set_title('a. Autumn Sea Ice Thickness from Satellites',size=8, weight='bold')
...
### Legends ###
ax1.legend(fontsize=7, loc='lower left',bbox_to_anchor=(0.54,0.64), framealpha=1, edgecolor='w', ncol=1)
