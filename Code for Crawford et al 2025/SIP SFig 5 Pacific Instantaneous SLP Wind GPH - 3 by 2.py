...
# Date Variables
times = [ [1983,12,16,0,0,0], [1983,12,18,0,0,0], [1983,12,20,0,0,0]]
dateref = [1900,1,1,0,0,0]

# Plotting Variables
prj = ccrs.NorthPolarStereo(central_longitude=180, true_scale_latitude=70)
extent = [-3500000,3500000,-6500000,-1000000] # Left, Right, Bottom, Top
cbbox = [32,70,90,-100]
...
slpbreaks = np.arange(96400,106000,200)
slpcbbreaks = np.arange(96400,106000,800)
slplabels = np.arange(964,1060,8)

gphbreaks = np.arange(4860,5900,60)*9.81
gphcbbreaks = np.arange(4860,5900,180)*9.81
gphlabels = np.arange(4860,5900,180)
...
for i in range(3):
    ...
    uvpsn = xr2.sel(time=timestr)
    urot, vrot = md.rotateCoordsAroundOrigin(uvpsn['u10'], uvpsn['v10'], (era5prj['longitude']-(180))*np.pi/-180)
    vec = ax1.quiver(era5prj['longitude'], era5prj['latitude'],urot.data, vrot.data, 
        transform=ccrs.PlateCarree(), regrid_shape=21, width=0.005, scale=300)
