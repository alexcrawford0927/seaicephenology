...
# Date Variables
times = [ [2010,12,15,0,0,0], [2010,12,17,0,0,0], [2010,12,19,0,0,0]]
dateref = [1900,1,1,0,0,0]

# Plotting Variables
prj = ccrs.NorthPolarStereo(central_longitude=-90, true_scale_latitude=70)
extent = [-2000000,3500000,-4500000,0] # Left, Right, Bottom, Top
cbbox = [40,80,-160,-40]
...
slpbreaks = np.arange(98000,106000,200)
slpcbbreaks = np.arange(98000,106000,800)
slplabels = np.arange(980,1060,8)

gphbreaks = np.arange(5040,5850,60)*9.81
gphcbbreaks = np.arange(5040,5850,120)*9.81
gphlabels = np.arange(5040,5850,120)
